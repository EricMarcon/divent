...
- log-normal ("lnorm") distributions [@Preston1948] require argument `sd_lnorm` to set the standard deviation of the log-abundances. The expectation of the distribution is computed from the other arguments.
- log-series ("lseries") distributions [@Fisher1943] rely on the `fisher_alpha` argument. They ignore `species_number` because the number of species is given by `fisher_alpha` and `size`.
- geometric ("geom") distributions [@Motomura1932] have argument `prob_geom`. This distribution is not stochastic.
...
Post-simulation processing of such spatialized communities include thinning to reduce the local average basal area.
