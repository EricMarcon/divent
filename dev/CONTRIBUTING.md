...
[Issues](https://github.com/ericmarcon/divent/issues) that might help
...
- To see if other users have experienced (and potentially already
  ...
  tracker](https://github.com/ericmarcon/divent/issues?q=label%3Abug).

### How do I submit a bug report?

We use [GitHub Issues](https://github.com/ericmarcon/divent/issues) to
...
- Make sure that you are using the latest version of `divent`.
- Read the [documentation](https://ericmarcon.github.io/divent/)
  carefully and find out if the functionality is already covered.
- Perform a [search](https://github.com/ericmarcon/divent/issues) to see
...
Issues](https://github.com/ericmarcon/divent/issues).
...
7.  Submit a [Pull
    ...
    on the [original
    repo](https://github.com/ericmarcon/divent/compare).
...
`DESCRIPTION` file under the section `Config/Needs/website` (only if
this package is not already listed under the section `Imports` or
`Suggests`).
...
forget to add it in the `DESCRIPTION` file under the section `Suggests`
and rely on [`require()`](https://rdrr.io/r/base/library.html) rather
than [`library()`](https://rdrr.io/r/base/library.html) to load the
package. For example, to use function `bar()` from package `foo`, write:
