...
- Add an `autoplot()` able to plot $\alpha$, $\beta$ and/or $\gamma$ profiles, similar to that of entropart.

- Add `profile_beta_*()` functions.
  Accept an object created by `div_part()` as input.
